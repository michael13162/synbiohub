...
let v1Bits = v1.split('.');
let v2Bits = v2.split('.');
let v1OriginalLength = v1Bits.length;
let v2OriginalLength = v2Bits.length;
...
for (let i = 0; i < v1Bits.length; i++) {
let targetLength = Math.max(v1Bits[i].length, v2Bits[i].length);
...
let transformed1 = v1Bits.join('.');
let transformed2 = v2Bits.join('.');
