...
let collectionQuery = 'PREFIX dcterms: <http://purl.org/dc/terms/> PREFIX sbol2: <http://sbols.org/v2#> SELECT ?object ?name WHERE { ?object a sbol2:Collection . OPTIONAL { ?object dcterms:title ?name . } }';
let rootCollectionQuery = loadTemplate('sparql/RootCollectionMetadata.sparql', {});

return sparql.queryJson(rootCollectionQuery, req.user.graphUri).then((collections) => {
let rootCollections = collections.map((collection) => {
return {
uri: collection.Collection,
name: collection.name ? collection.name : collection.Collection.toString(),
};
});

rootCollections.sort(sort.byNames);
