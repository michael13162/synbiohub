...
module.exports = function(req, res) {
  ...
  let limit = config.get('defaultLimit');
  if (req.query.limit) {
...
collectionIcon = collectionIcons[collection.uri];
    ...
    locals.meta = {
      ...
      members: members.map((member) => {
        ...
        let memberTypeUrl = member.type;
        let memberType = typeLocalPart;
    ...
    locals.meta.members.forEach((annotation) => {
      let namespaces = listNamespaces(config.get('namespaces')).filter(function(namespace) {
        return annotation.type.indexOf(namespace.uri) === 0;
      });
      if (namespaces.length != 0) {
        let namespace = namespaces.sort((a, b) => a.uri.length - b.uri.length)[0];
        let prefixedName = namespace.prefix + ':' + annotation.type.slice(namespace.uri.length);
    ...
    getOwnedBy(uri, graphUri).then((ownedBy) => {
      if (!metaData.remote && req.user) {
	    ...
	    let annotations = ownedBy.map((owner) => {
