...
module.exports = function(req, res) {
  if (req.method === 'POST') {
    let submissionData = {
      ...
      overwriteMerge: req.body.overwriteMerge || '',
    };
...
function clonePost(req, res, submissionData) {
  ...
  let overwriteMerge = req.body.overwriteMerge;
...
submissionData.version === req.params.version) {
  ...
  fetchSBOLObjectRecursive(uri, req.user.graphUri).then((result) => {
    ...
    let newUri = config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.username) + '/' + submissionData.id + '/' + submissionData.id + '_collection' + '/' + submissionData.version;
