...
let resolveBatch = config.get('resolveBatch');
let webOfRegistries = config.get('webOfRegistries');
...
// console.log(sbol.unresolvedURIs.length + ' unresolved URI(s)')
  ...
  if (sbol.unresolvedURIs.length === 0) {
    ...
    let toResolve = sbol.unresolvedURIs.filter((uri) => !sbol._resolving[uri] &&
!skip.has(uri.toString()) && uri.toString().startsWith(databasePrefix))
    ...
    retrieveSBOL(graphUri, sbol, type, toResolve, (err) => {
      ...
      //
      let uriStrings = sbol.unresolvedURIs.map((uri) => uri.toString());

      for (let i = 0; i < uriStrings.length; ++ i) {
        let uri = uriStrings[i];
...
function retrieveSBOL(graphUri, sbol, type, uris, next) {
  ...
  let countQuery = sparqlDescribeSubjects(sbol, type, uris, true);

  let query = sparqlDescribeSubjects(sbol, type, uris, false);
